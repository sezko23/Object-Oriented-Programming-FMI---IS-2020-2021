...
std::istream& operator >> (std::istream& is, Planet& _planet) {
		...
		std::cout << "Numbers for planet type:" << std::endl;
		std::cout << "carbon = 0, desert = 1, helium = 2, ice = 3, lava = 4, ocean = 5 " << std::endl;
		std::cout << "Enter number for planet type: ";
		is >> type;
		_planet.type = planet_type(type);
